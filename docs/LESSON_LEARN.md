...
> **关于 v1.1.x 末态的 3 个失败测试**：全部指向同一根因——v1.1.x 期间名单长度与单测期望之间的不一致。属 v1.1.x 末态的旧事故，与当前 release (v1.2.0) 已无关。**v1.2.0 commit `b2ab140` 收口**：运行时移除名单改为工作区邀请制；本条为历史教训归档，**不再涉及具体名单 / 人名**。
>
> **2026-09-02 补充核实**：放宽钩子超时后单独重跑，确认**真实失败断言共 5 个，全部同一根因**（v1.1.x 历史名单长度不一致：3 个 + 2 个 `GET /api/users` 长度期望）。详见 §7.9。
...
**结论**：v1.1.x 时代的固定人员名单（v1.1.0 20 人 → v1.1.1 末态 21 人）vs 单测期望之间的长度不一致是 v1.1.x 期间真实失败断言的根因。**已于 v1.2.0 commit `b2ab140` 收口**：运行时移除该名单、改为 workspace 邀请制；本条为历史教训归档，**不再涉及具体名单 / 人名**。放宽钩子超时的全量测试在 v1.1.x 末态为 **252 passed / 0 failed（13 套件全绿）**。
...
**现象**：单次消息里对**同一文件**连发多条编辑（不同段落、不重叠位置），工具**每条都返回成功**，但最终落盘版本只有**最后几条**生效——前面几条静默丢失。
...
### 7.17 左右并排双滚动容器：两侧「底部条」必须同高，否则 K16 滚动同步差几像素

**现象**：给表格加顶部筛选条后，实测甘特侧 `clientHeight` 比表格侧多 **2px**（733 vs 731），滚到底时两侧 `scrollTop` 对不齐。排查发现 `scrollHeight` 完全相等（都是 2020），差异只出在 `clientHeight`。

**根因**：本工具左侧表格与右侧甘特图是**左右并排、各一根独立滚动条**，靠「内容总高相等」保证滚动同步（K16 行高约束）。但两侧的**底部固定条**高度不一致 —— 甘特图底部有个 24px 图例条（`py-1` 的 flex 行），左侧的「只读提示」条却只在只读态渲染（26px），编辑态下左侧底部没有等高的占位条 → 差 2px。

**教训**：这类「两侧镜像布局」的改动，**任何一侧新增/删除固定条（顶部工具条、底部图例、只读提示）都要同步两侧**，否则 `clientHeight` 一差、滚到底就错位。本轮顺带把两侧底部条统一成**固定等高**（编辑态左侧也有同高占位条），而不是「按内容自适应」。

**排查手法**：Playwright 里 `scrollHeight` / `clientHeight` 分开打。`scrollHeight` 相等但 `clientHeight` 不等 → 是**容器 chrome（固定条/padding/border）**的问题，不是行数据的问题。

### 7.18 过滤做在「视图层」而非数据层：共用 `useVisibleTasks`，天然两侧同步 + 不污染导出

**背景**：U03 加筛选。最容易想歪的方案是「筛出 tasks 子集再喂给表格和甘特图」，但那会破坏 WBS 树、还要重算排程、还得决定导出时要不要带着过滤结果。正确做法：**过滤是纯视图态**。

**正解**：
- 过滤只作用于 `computeVisibleTasks(tasks, keepIds?)` 的第二个参数，**不修改 `plan.tasks`、不写历史、不进 MSPDI/CSV 导出**。
- `TaskTable` 与 `GanttChart` 本就共用同一个 `useVisibleTasks()`（K16 行对齐的既有约束），所以过滤**天然两侧同步**，甘特图侧零额外代码。
