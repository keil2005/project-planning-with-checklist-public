...
- [x] **祖先链**（用户裁定：显示匹配行 + 祖先链）：命中行的父/祖任务一并保留，即使祖先不匹配，WBS 树不断链，折叠行为不受影响。
- [x] **计数与清除**：工具条常显「显示 N / 共 M 行」；已筛列以 chips 显示、可单项清除；「清除全部」一次清空列筛选 + Assign to me；漏斗在有筛选时高亮。
- [x] **过滤态编辑约束**：过滤激活时「新增一行」（底部 + 行内）禁用，清除后恢复。
- [x] **不记忆**（用户裁定）：切计划/新建/导入/预览/恢复版本时重置筛选；刷新即清空。
- [x] **两侧等高（K16）**：过滤同步作用于表格与甘特图（共用 `useVisibleTasks`）；顶部两侧加等高筛选条、底部图例条与只读提示条统一等高，滚动同步严格成立。
- [x] **过滤是纯视图态**：不进 `plan.tasks`、不写历史、不进 MSPDI/CSV 导出。
- [x] **文档**：`docs/prd_increment_filter.md`、`docs/design_increment_filter.md`。
- **质量门禁**：`tsc --noEmit` 0 错误；全量单测 18 套件 **337 passed / 0 failed**（312 → 337，新增 25 例）。
- **真实浏览器实测**（Chromium / Project-A 59 行）：7 漏斗；负责人面板 16 项（含 `(空白)`）；Assign to me 59→9 且祖先链完整（Design/Prototype/Validation/ETS0 等父任务保留）、再点恢复 59；任务名称「DQ」→1 行、开始「不早于 2026-09-01」→0 行（多列 AND）；chips 单项清除 + 清除全部；过滤态新增行禁用；两侧 `clientHeight`/`scrollHeight` 相等（733/2020）、滚到底 scrollTop 一致；**0 控制台错误**。
