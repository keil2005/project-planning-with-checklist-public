...
describe('datetime · ENDPOINT 语义', () => {
  it('2026-08-26 + 1m = 2026-09-23（20 工作日，WEEKEND_ONLY）', () => {
    expect(addDuration('2026-08-26', { value: 1, unit: 'm' }, 1, WEEKEND_ONLY)).toBe('2026-09-22');
  });

  it('2026-01-31 + 1m = 2026-02-28（WEEKEND_ONLY：首工作日为 02-02，第20工作日 02-27，end 02-28）', () => {
    expect(addDuration('2026-01-31', { value: 1, unit: 'm' }, 1, WEEKEND_ONLY)).toBe('2026-02-27');
  });

  it('5d / 2w 跨度（WEEKEND_ONLY）', () => {
    expect(addDuration('2026-08-26', { value: 5, unit: 'd' }, 1, WEEKEND_ONLY)).toBe('2026-09-01');
    expect(addDuration('2026-08-26', { value: 2, unit: 'w' }, 1, WEEKEND_ONLY)).toBe('2026-09-08');
  });

  it('subDuration 为 addDuration 的逆（WEEKEND_ONLY round-trip）', () => {
    expect(subDuration('2026-09-22', { value: 1, unit: 'm' }, WEEKEND_ONLY)).toBe('2026-08-26');
  });
  ...
  it('diffDuration 单位优先级 m > w > d（WEEKEND_ONLY）', () => {
    expect(diffDuration('2026-08-26', '2026-09-22', WEEKEND_ONLY)).toEqual({ value: 1, unit: 'm' });
    expect(diffDuration('2026-08-26', '2026-09-08', WEEKEND_ONLY)).toEqual({ value: 2, unit: 'w' });
    expect(diffDuration('2026-08-26', '2026-08-28', WEEKEND_ONLY)).toEqual({ value: 3, unit: 'd' });
  });
});
...
describe('scheduler · 三选二决策矩阵', () => {
  it('#2 S+D', () => {
    ...
    const r = sched(plan);
    expect(r.computed['T-0001'].end).toBe('2026-09-22');
    expect(r.computed['T-0001'].derivedFrom).toBe('INPUT');
  });

  it('#3 E+D 反推开始', () => {
    const plan = makePlan([task('T-0001', 'A', { input: { start: null, end: '2026-09-22', duration: '1m' } })]);
    expect(sched(plan).computed['T-0001'].start).toBe('2026-08-26');
  });

  it('#1 S+E 推时长', () => {
    const plan = makePlan([task('T-0001', 'A', { input: { start: '2026-08-26', end: '2026-08-30', duration: null } })]);
  ...
  it('#5 仅 S → 缺省 1d + warn 1017', () => {
    ...
    const r = sched(plan);
    expect(r.computed['T-0001'].end).toBe('2026-08-26');
  ...
  it('#15 三填冲突 → error 1003；一致 → warn 1018', () => {
    const bad = makePlan([task('T-0001', 'A', { input: { start: '2026-08-26', end: '2026-09-23', duration: '1m' } })]);
    expect(sched(bad).diagnostics.some((d) => d.code === ErrCode.ERR_OVER_CONSTRAINED)).toBe(true);

    const good = makePlan([task('T-0001', 'A', { input: { start: '2026-08-26', end: '2026-09-22', duration: '1m' } })]);
  ...
  it('FF+1w 依赖驱动结束', () => {
    ...
    const c = sched(makePlan([a, b])).computed;
    expect(c['T-0002'].end).toBe('2026-09-08');
  });
...
describe('scheduler · 父子 rollup 与环检测', () => {
  it('两级父任务 rollup', () => {
    ...
    expect(c['T-0002'].end).toBe('2026-09-01');
    expect(c['T-0001'].start).toBe('2026-08-26');
    expect(c['T-0001'].end).toBe('2026-09-28');
  ...
  it('projectStart / projectEnd', () => {
    ...
    expect(r.projectEnd).toBe('2026-09-28');
  });
});
